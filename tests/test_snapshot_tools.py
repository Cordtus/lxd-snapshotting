...
class SnapshotToolTests(unittest.TestCase):
    ...
    def test_upload_dry_run_marks_transfer_rsyncs_and_triggers_finalizer(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("cosmos-snapshot-remote prepare --chain examplechain", lines[0])
            self.assertIn("rsync -a --partial --info=progress2", lines[1])
            self.assertIn("--delete --numeric-ids", lines[1])
            self.assertIn("--rsync-path=cosmos-snapshot-remote rsync --chain examplechain", lines[1])
            ...
            self.assertIn("cosmos-snapshot-remote finalize --chain examplechain", lines[2])

    def test_upload_executes_prepare_rsync_finalize_in_order(self):
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            ...
            self.assertEqual([entry[0] for entry in entries], ["ssh", "rsync", "ssh"])
            self.assertIn("cosmos-snapshot-remote prepare --chain examplechain", entries[0][1][-1])
            self.assertTrue(any(arg.startswith("--rsync-path=cosmos-snapshot-remote rsync --chain examplechain") for arg in entries[1][1]))
            self.assertIn("cosmos-snapshot-remote finalize --chain examplechain", entries[2][1][-1])
    ...
    def test_upload_does_not_finalize_after_rsync_failure(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            bin_dir.mkdir()
            self.write_fake_command(bin_dir, "ssh", log_path)
            self.write_fake_command(bin_dir, "rsync", log_path, exit_code=23)
            config = tmp_path / "upload.env"
            config.write_text(
                "\n".join([
                    "CHAIN=\"examplechain\"",
            ...
            entries = self.read_json_lines(log_path)
            self.assertEqual([entry[0] for entry in entries], ["ssh", "rsync"])
            self.assertIn("cosmos-snapshot-remote prepare --chain examplechain", entries[0][1][-1])
    ...
    def test_remote_wrapper_rejects_rsync_outside_allowed_chain_path(self):
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            access = tmp_path / "access.json"
            remote_config = tmp_path / "remote.env"
            ...
            self.assertNotEqual(result.returncode, 0)
            self.assertIn("outside allowed ingest path", result.stderr)
